...
static char current_cd[MAX_STRING] = "\0";	//init as null
static char current_cat[MAX_STRING];		//Global var

const char *title_file = "title.cdb";		//const val
...
int main()
{
	...
	do {
		choice = getchoice("Option:", current_cd[0] ? extended_menu : main_menu);
		switch(choice) {
			case 'q':
				break;
			case 'a':
...
int getchoice(char *greet, char *choices[])
{
	...
	while(key != 'q' && key != KEY_ENTER && key != '\n') {
		...
		selected = *choices[selected_row];	//choose string first char
...
void draw_menu(char *options[], int current_highlight, int start_row, int start_col)
{
	...
	while(*option_ptr) {
		...
		//txt_ptr++;	//will lost the first char
...
void remove_cd()
{
	...
	unlink(title_file);	//delete the titles file
...
void find_cd()
{
	...
	if(titles_fp) {
		while (fgets(entry, MAX_ENTRY, titles_fp)) {
			catalog = entry;
			fprintf(stdout, "before strstr: %s\n", catalog);
			if(found == strstr(catalog, ",")) {     //此处有问题，总是返回false，待处理
				*found = '\0';
				title = found + 1;
				fprintf(stdout, "title: %s, found:%s", title, found);
				if (found == strstr(title, ",")) {
					*found = '\0';
					if(found == strstr(title, match)) {
						fprintf(stdout, "title: %s, catalog: %s, match: %s", title, catalog, match);
						count++;
