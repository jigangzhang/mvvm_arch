...
int read_request_from_client(message_db_t *rec_ptr) {
	...
	if(server_fd != -1) {
		read_byes = read(server_fd, rec_ptr, sizeof(*rec_ptr));
		if(read_byes == 0) {
			(void)close(server_fd);
			if((server_fd = open(SERVER_PIPE, O_RDONLY)) == -1) {
				if(errno != EINTR)
...
int start_resp_from_server(void) {
	...
	if(client_fd != -1) {
		client_write_fd = open(client_pipe_name, O_WRONLY); //no use, jingzhen
		if(client_write_fd != -1)
			return 1;
		(void)close(client_fd);
		client_fd = -1;
...
int read_resp_from_server(const message_db_t *rec_ptr) {
	int read_bytes;
	int return_code = 0;
